...
import { getLiveTradingToolById, updateLiveTradingToolById } from 'apiSdk/live-trading-tools';
...
function LiveTradingToolEditPage() {
  ...
  const handleSubmit = async (values: LiveTradingToolInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateLiveTradingToolById(id, values);
      mutate(updated);
      resetForm();
