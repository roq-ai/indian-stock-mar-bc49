...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getIndicatorById() {
    const data = await prisma.indicator.findFirst(convertQueryToPrismaUtil(req.query, 'indicator'));
    return res.status(200).json(data);
  }

  async function updateIndicatorById() {
    await indicatorValidationSchema.validate(req.body);
